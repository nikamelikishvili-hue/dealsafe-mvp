...
const seed: Deal[] = [{
  ...
  sellerVerification: 'verified', agreementVersion: 1, createdAt: new Date().toISOString(), expiresAt: new Date(Date.now()+7*24*60*60*1000).toISOString()
}];

export const demoRepository: DealRepository = {
  async list() { return [...seed]; },
  async create(draft) {
    const deal: Deal = { id: crypto.randomUUID(), publicId: `DS-${Math.random().toString(36).slice(2, 8).toUpperCase()}`, title: draft.title, description: draft.description, priceCents: toMinorUnits(draft.price, draft.currency), currency: draft.currency, condition: draft.condition, serialNumber: draft.serialNumber, deliveryMethod: draft.deliveryMethod, status: 'published', sellerName: 'You', sellerVerification: 'pending', agreementVersion: 1, createdAt: new Date().toISOString(), expiresAt: new Date(Date.now()+(draft.expiresInDays||7)*24*60*60*1000).toISOString() };
