...
function AdminEvidenceReview({dispute,session}:{dispute:AdminDispute;session:StoredSession}){
  const [items,setItems]=useState<DealEvidence[]>([]);const [selected,setSelected]=useState<DealEvidence|null>(null);const [message,setMessage]=useState('');
  useEffect(()=>{let current=true;setMessage('');setSelected(null);listDealEvidence(session,dispute.deal_id).then(next=>{if(current)setItems(next)}).catch(error=>{if(current)setMessage(error instanceof Error?error.message:'Could not load evidence')});return()=>{current=false}},[dispute.deal_id,session.accessToken]);
  const verified=(id:string,checkedAt:string)=>setItems(current=>current.map(item=>item.id===id?{...item,integrity_status:'verified',integrity_checked_at:checkedAt}:item));
  return <div className="admin-evidence-review"><div className="admin-evidence-heading"><b>{t('Evidence files')}</b><span>{items.length} {t(items.length===1?'file':'files')}</span></div>{message&&<div className="notice">{t(message)}</div>}{items.length?items.map(item=><article key={item.id}><div className="admin-evidence-preview"><FileCheck2 size={20}/></div><div><b>{t(evidenceLabels[item.evidence_type]||'Other evidence')}</b><span>{item.file_name||t('Uploaded file')} · {item.uploader_role} · {item.sha256?`SHA-256 ${item.sha256.slice(0,12)}…`:t('Fingerprint unavailable')}</span><span>{t(item.integrity_status==='verified'?'Integrity verified':item.integrity_status==='unverified'?'Integrity check required':'File blocked by integrity check')}</span></div>{item.scan_status==='clean'?<button type="button" className="secondary" onClick={()=>setSelected(item)}>{t('Verify and open')}</button>:<em>{t('Security review required')}</em>}</article>):<p>{t('No evidence uploaded yet.')}</p>}{selected&&<EvidenceViewer item={selected} label={evidenceLabels[selected.evidence_type]||'Other evidence'} session={session} onClose={()=>setSelected(null)} onVerified={checkedAt=>verified(selected.id,checkedAt)}/>}</div>
}
...
function EvidencePanel({deal,session,onChanged}:{deal:Deal;session:StoredSession;onChanged?:()=>void}){
  ...
  const [files,setFiles]=useState<File[]>([]);const [items,setItems]=useState<DealEvidence[]>([]);const [selected,setSelected]=useState<DealEvidence|null>(null);const [busy,setBusy]=useState(false);const [message,setMessage]=useState('');
  ...
  useEffect(()=>{setEvidenceType(role==='seller'?'seller_packing_video':'buyer_unboxing_video');setFiles([]);setSelected(null);setMessage('');void load()},[deal.id,session.accessToken,role]);
  const upload=async(event:React.FormEvent)=>{event.preventDefault();if(!files.length)return;setBusy(true);setMessage('');try{for(const file of files)await uploadDealEvidence(session,deal.id,role,evidenceType,file);setFiles([]);await load();onChanged?.();setMessage('Security scan passed. Evidence was saved privately to this deal record.')}catch(error){setMessage(error instanceof Error?error.message:'Could not upload evidence')}finally{setBusy(false)}};
  const verified=(id:string,checkedAt:string)=>setItems(current=>current.map(item=>item.id===id?{...item,integrity_status:'verified',integrity_checked_at:checkedAt}:item));
  const options=role==='seller'?sellerOptions:buyerOptions;
  const acceptedFiles=evidenceInputAccept(role,evidenceType);
  return <section id="deal-evidence-vault" className="evidence-panel no-print"><div className="evidence-heading"><ShieldCheck/><div><p className="eyebrow">{t('Evidence vault')}</p><h2>{t(role==='seller'?'Document the package before shipping':'Document the item when it arrives')}</h2><span>{t(role==='seller'?'Record the item, serial number, and sealed package.':'Record the delivery, packaging, and unboxing before using the item.')}</span></div></div><form className="evidence-form" onSubmit={upload}><label>{t('Evidence type')}<select value={evidenceType} onChange={event=>{setEvidenceType(event.target.value as EvidenceType);setFiles([])}}>{options.map(option=><option key={option} value={option}>{t(evidenceLabels[option])}</option>)}</select></label><label className="evidence-picker">{t('Choose photos or video')}<input type="file" accept={acceptedFiles} multiple onChange={event=>{setFiles(Array.from(event.target.files||[]));event.currentTarget.value=''}}/><small>{files.length?`${files.length} ${t(files.length===1?'file selected':'files selected')}`:t('Photos up to 10 MB; videos up to 50 MB')}</small></label>{files.length>0&&<div className="evidence-file-list">{files.map((file,index)=><span key={`${file.name}-${index}`}><Package size={15}/>{file.name}<small>{Math.ceil(file.size/1024/1024)} MB</small></span>)}</div>}<button className="primary" disabled={busy||!files.length}>{busy?t('Scanning and saving…'):t('Scan and save evidence')}</button></form><p className="evidence-scan-note"><ShieldCheck size={15}/>{t('Files enter an isolated quarantine and are available only after type, size, and malware checks pass.')}</p>{message&&<div className="notice">{t(message)}</div>}<div className="evidence-list"><div className="evidence-list-heading"><b>{t('Saved evidence')}</b><span>{items.length} {t(items.length===1?'file':'files')} · {t('Private to the deal participants')}</span></div>{items.length?items.map(item=><article key={item.id}><BadgeCheck size={17}/><div><b>{t(evidenceLabels[item.evidence_type]||'Other evidence')}</b><span>{item.file_name||t('Uploaded file')} · {formatDateTime(item.created_at)}</span><span>{item.sha256?`SHA-256 ${item.sha256.slice(0,12)}…`:t('Fingerprint unavailable')} · {t(item.integrity_status==='verified'?'Integrity verified':item.integrity_status==='unverified'?'Integrity check required':'File blocked')}</span></div><em className={item.scan_status==='clean'&&item.integrity_status!=='missing'&&item.integrity_status!=='mismatch'&&item.integrity_status!=='invalid'?'is-clean':'needs-review'}>{t(item.scan_status!=='clean'?'Security review required':item.integrity_status==='verified'?'Verified':'Recheck on open')}</em>{item.scan_status==='clean'&&<button type="button" className="secondary evidence-open" onClick={()=>setSelected(item)}>{t('Verify and open')}</button>}</article>):<p>{t('No evidence uploaded yet.')}</p>}</div><p className="evidence-note"><LockKeyhole size={15}/>{t('Evidence is append-only, access is logged, and each viewing link expires after 60 seconds.')}</p>{selected&&<EvidenceViewer item={selected} label={evidenceLabels[selected.evidence_type]||'Other evidence'} session={session} onClose={()=>setSelected(null)} onVerified={checkedAt=>verified(selected.id,checkedAt)}/>}</section>;
}
...
function SavedDraftPanel({deal,session,onUpdated}:{deal:Deal;session:StoredSession;onUpdated:(deal:Deal)=>void}){
  ...
  const [edit,setEdit]=useState<DealDraft>({title:deal.title,description:deal.description,price:amountForInput(deal.priceCents,deal.currency),currency:deal.currency,condition:deal.condition,serialNumber:deal.serialNumber?.slice(-4)||'',deliveryMethod:deal.deliveryMethod,expiresInDays:remainingDays,catalog:deal.catalog});
...
export function App() {
  ...
  const finishSignedOutSession=()=>{setSession(null);setMfaLogin(null);setIsAdmin(false);setView('home')};
