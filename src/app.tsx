...
export function App() {
  ...
  const openCreate=()=>{setAuthMessage('');if(!user){setReturnAfterAuth('create');setView('auth');return}setDealTemplate('phone');setCreateStep(1);setReviewingDraft(false);setView('create')};
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      {view==='create'&&<section id="create-deal-flow" className="create-flow-shell">
        ...
        {!reviewingDraft&&createStep===1&&<div className="create-step-layout">
          ...
          <section className="form-wrap create-step-card"><form id="create-step-1" onSubmit={event=>{event.preventDefault();goToCreateStep(2)}}><label>{t('Item title')}<input required minLength={3} maxLength={120} placeholder={selectedDealTemplate.titlePlaceholder} value={draft.title} onChange={event=>setDraft({...draft,title:event.target.value})}/><small>{t('Use the brand, model, and one detail that helps identify the item.')}</small></label><div className="two"><label>{t('Price')}<span className="price-currency-controls"><input required min={currencyStep(draft.currency)} step={currencyStep(draft.currency)} type="number" placeholder="780" value={draft.price} onChange={event=>setDraft({...draft,price:event.target.value})}/><span className="currency-label">USD</span></span></label><label>{t('Condition')}<select value={draft.condition} onChange={event=>setDraft({...draft,condition:event.target.value as DealDraft['condition']})}><option value="Like new">{t('Like new')}</option><option value="Good">{t('Good')}</option><option value="Fair">{t('Fair')}</option></select></label></div></form></section>
        </div>}
        {!reviewingDraft&&createStep===2&&<section className="form-wrap create-step-card"><form id="create-step-2" onSubmit={event=>{event.preventDefault();goToCreateStep(3)}}><div className="create-step-guidance"><ShieldCheck/><div><b>{t('What the buyer needs to know')}</b><span>{t(selectedDealTemplate.descriptionPrompt)}</span></div></div><label>{t('Known condition and defects')}<textarea required minLength={20} placeholder={t(selectedDealTemplate.descriptionPrompt)} value={draft.description} onChange={event=>setDraft({...draft,description:event.target.value})}/><small>{draft.description.trim().length}/20 · {t('Describe wear, repairs, or defects.')}</small></label><label>{t(selectedDealTemplate.identifierLabel)}<input maxLength={40} pattern={selectedDealTemplate.identifierPattern} title={t(selectedDealTemplate.identifierHelp)} placeholder={t(selectedDealTemplate.identifierPlaceholder)} spellCheck={false} aria-invalid={identifierEntered&&!identifierValid} value={draft.serialNumber} onChange={event=>setDraft({...draft,serialNumber:dealTemplate==='vehicle'?event.target.value.toUpperCase():event.target.value})}/><small className={`identifier-feedback ${identifierEntered?(identifierValid?'valid':'invalid'):''}`}>{t(identifierEntered?(identifierValid?'Format looks correct. This checks format only, not ownership or authenticity.':selectedDealTemplate.identifierHelp):'Stored privately; only last characters shown')}</small></label><div className="two"><label>{t('Handoff')}<select value={draft.deliveryMethod} onChange={event=>setDraft({...draft,deliveryMethod:event.target.value as DealDraft['deliveryMethod']})}><option value="Meet in person">{t('Meet in person')}</option><option value="Ship to buyer">{t('Ship to buyer')}</option></select></label><label>{t('Offer valid for')}<select value={draft.expiresInDays||7} onChange={event=>setDraft({...draft,expiresInDays:Number(event.target.value)})}><option value={1}>{t('1 day')}</option><option value={3}>{t('3 days')}</option><option value={7}>{t('7 days')}</option><option value={14}>{t('14 days')}</option><option value={30}>{t('30 days')}</option></select></label></div><div className="notice"><ShieldCheck/><span>{t('The Deal Link is not public until you confirm.')}</span></div></form></section>}
        ...
        {reviewingDraft&&<CreateDealReview draft={draft} photos={photos} creating={creating} onEdit={()=>{setReviewingDraft(false);setCreateStep(3)}} onSaveDraft={saveDraft} onPublish={create}/>}
        {!reviewingDraft&&<div className="create-action-dock" role="region" aria-label={t('Create deal action')}><div><small>{t(createStepMeta[createStep].eyebrow)}</small><strong>{t(createStepMeta[createStep].dock)}</strong><span>{t(createStep===3?'Photos are optional. Continue when the record looks clear.':'Your progress stays here while you complete the next short step.')}</span></div><button type="button" className="primary" onClick={()=>{const form=document.getElementById(`create-step-${createStep}`) as HTMLFormElement|null;form?.requestSubmit()}}>{t(createStepMeta[createStep].action)}<ArrowRight size={18}/></button></div>}
      </section>}
