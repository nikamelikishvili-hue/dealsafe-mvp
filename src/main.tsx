...
import { getAppLanguage, initializeI18n, t } from './i18n';
...
function AdminRevenueCenter({session,onOpenDeal}:{session:StoredSession;onOpenDeal:(deal:Deal)=>void}){
  ...
  const cards=summary?[
    ...
    {label:'DealSafe commission earned',value:money(summary.total_commission_earned_cents),detail:`${summary.released_count} ${t(summary.released_count===1?'released deal':'released deals')}`,tone:'green'},
...
function TimelinePanel({deal,session}:{deal:Deal;session:StoredSession}){
  ...
  const share=async()=>{const history=[`DealSafe · ${deal.publicId}`,deal.title,dealPrice(deal),'',...events.slice().reverse().map(event=>`${formatDateTime(event.created_at)} · ${friendlyEvent(event.event_type)} · ${t(event.is_mine?'By you':'By the other party')}`)].join('\n');try{if(navigator.share)await navigator.share({title:`DealSafe · ${deal.publicId}`,text:history,url:`${location.origin}/?deal=${deal.publicId}`});else await navigator.clipboard.writeText(history)}catch(error){if(error instanceof Error&&error.name!=='AbortError')setError('Could not share this link.')}};
...
function DealActionPlanCard({deal,session,onSync}:{deal:Deal;session:StoredSession;onSync:(plan:DealActionPlan)=>void}){
  ...
  const steps=[
    ...
    {label:'Payment acknowledged',done:plan.payment_received||completed},
  ...
  if(plan.deal_status==='cancelled')action={label:'This deal is cancelled.'};
  ...
  else if(!completed&&!plan.payment_received){
    if(!plan.payment_method_recorded)action=plan.viewer_role==='seller'?{label:'Choose payment method',target:'payment-status-panel'}:{label:'Waiting for seller to choose payment method'};
    else if(!plan.payment_method_confirmed)action=plan.viewer_role==='buyer'?{label:'Confirm payment method',target:'payment-status-panel'}:{label:'Waiting for buyer to confirm payment method'};
    else if(plan.viewer_role==='seller')action={label:'Confirm payment received',target:'payment-status-panel'};
    else if(!plan.payment_marked_sent)action={label:'Mark payment as sent',target:'payment-status-panel'};
    else action={label:'Waiting for seller to confirm payment received'};
  }
    ...
    if(!plan.delivery_address_ready)action=plan.viewer_role==='buyer'?{label:'Add delivery address',target:'shipping-panel'}:{label:'Waiting for buyer to add delivery address'};
    else if(!plan.shipment_status)action=plan.viewer_role==='seller'?{label:'Add tracking details',target:'shipping-panel'}:{label:'Waiting for the seller to add tracking information.'};
...
function printProtectedPaymentReceipt(deal:Deal,payment:ProtectedPaymentStatus){
  ...
  const rows=[
    ...
    [t('DealSafe service fee'),money(payment.platform_fee_cents)],
...
function ShippingPanel({deal,session,paymentReady,evidenceRevision,onDelivered}:{deal:Deal;session:StoredSession;paymentReady:boolean;evidenceRevision:number;onDelivered:()=>void}){
  ...
  const saveAddress=async(e:React.FormEvent)=>{e.preventDefault();setSavingAddress(true);setMessage('');try{const storedAddress=[address.streetAddress.trim(),address.addressLine2.trim(),address.city.trim()].filter(Boolean).join('\n');await saveDealDeliveryDetails(session,deal.id,address.recipientName,storedAddress,address.country,address.instructions);await loadDelivery();setEditingAddress(false);setMessage('Address saved. The seller can now prepare the shipment.')}catch(error){setMessage(error instanceof Error?error.message:'Could not save delivery address')}finally{setSavingAddress(false)}};
  ...
  const addressIncomplete=address.recipientName.trim().length<2||address.streetAddress.trim().length<3||streetNumberMissing||address.city.trim().length<2||address.country.trim().length<2;
  return <section className="shipping-panel no-print"><div className="shipping-heading"><Truck/><div><p className="eyebrow">{t('Tracked delivery')}</p><h2>{t('Shipping & receipt')}</h2></div></div><div className="delivery-address-section"><div className="delivery-address-heading"><MapPin/><div><p className="eyebrow">{t('Protected delivery')}</p><h3>{t('Delivery address')}</h3><span>{t('Only the buyer and seller can view this address.')}</span></div></div>{delivery&&!editingAddress&&<div className="delivery-address-card"><div><span>{t('Recipient name')}</span><strong>{delivery.recipient_name}</strong><address>{delivery.full_address}<br/>{delivery.country}</address>{delivery.instructions&&<small>{t('Delivery instructions')}: {delivery.instructions}</small>}</div><div className="delivery-address-actions">{delivery.locked&&<em><LockKeyhole size={14}/>{t('Locked after shipping')}</em>}<button className="secondary" onClick={copyAddress}><Copy size={16}/>{t('Copy address')}</button>{deal.viewerRole==='buyer'&&!delivery.locked&&<button className="secondary" onClick={()=>setEditingAddress(true)}>{t('Edit address')}</button>}</div></div>}{deal.viewerRole==='buyer'&&(!delivery||editingAddress)&&<form className="delivery-address-form" onSubmit={saveAddress}><label>{t('Recipient name')}<input required minLength={2} maxLength={100} autoComplete="name" value={address.recipientName} onChange={e=>setAddress({...address,recipientName:e.target.value})}/></label><label>{t('Street address (number and name)')}<AddressAutocomplete streetAddressOnly placeholder={t('123 Main St')} value={address.streetAddress} onChange={streetAddress=>setAddress(current=>({...current,streetAddress}))} onAddressParts={parts=>setAddress(current=>({...current,streetAddress:parts.streetAddress||current.streetAddress,city:parts.city||current.city,country:parts.country||current.country}))}/>{streetNumberMissing&&<small className="address-validation">{t('Choose a specific address that includes a street number.')}</small>}</label><label>{t('Apartment / Suite / Unit (optional)')}<input maxLength={100} autoComplete="address-line2" value={address.addressLine2} onChange={e=>setAddress({...address,addressLine2:e.target.value})} placeholder={t('Apt 4B, Suite 210, Unit 3')}/></label><label>{t('City')}<input required minLength={2} maxLength={100} autoComplete="address-level2" value={address.city} onChange={e=>setAddress({...address,city:e.target.value})} placeholder={t('New York')}/></label><label>{t('Country or region')}<input required minLength={2} maxLength={80} autoComplete="country-name" value={address.country} onChange={e=>setAddress({...address,country:e.target.value})}/></label><label>{t('Delivery instructions (optional)')}<textarea maxLength={500} value={address.instructions} onChange={e=>setAddress({...address,instructions:e.target.value})} placeholder={t('Apartment, access code, or safe delivery note')}/></label><div className="delivery-form-actions">{delivery&&<button type="button" className="secondary" onClick={()=>setEditingAddress(false)}>{t('Go back')}</button>}<button className="primary" disabled={savingAddress||addressIncomplete}>{t(savingAddress?'Saving…':'Save delivery address')}</button></div></form>}{deal.viewerRole==='seller'&&!delivery&&<div className="shipping-wait">{t('Waiting for the buyer to add a delivery address.')}</div>}<p className="delivery-privacy"><LockKeyhole size={15}/>{t('This address is used only for this deal and is never shown on the public Deal Link.')}</p></div>{deal.viewerRole==='seller'&&!shipment&&<div className={`shipping-readiness ${readyToShip?'is-ready':''}`} aria-busy={checkingReadiness}><div className="shipping-readiness-heading"><ShieldCheck/><div><p className="eyebrow">{t('Shipping readiness')}</p><h3>{t(readyToShip?'Ready to ship':'Seller evidence is required before shipping.')}</h3><span>{t('Complete these checks before you mark the package as shipped.')}</span></div><strong>{completedReadinessSteps}/{readinessSteps.length} {t('steps complete')}</strong></div><div className="shipping-readiness-progress"><span style={{width:`${completedReadinessSteps/readinessSteps.length*100}%`}}/></div><div className="shipping-readiness-list">{readinessSteps.map(step=><div key={step.label} className={step.ready?'complete':'missing'}>{step.ready?<Check/>:<Clock3/>}<span>{t(step.label)}</span><em>{t(step.optional?'Not required':step.ready?'Ready':'Missing')}</em></div>)}</div>{checkingReadiness&&<div className="shipping-readiness-status">{t('Checking shipping readiness…')}</div>}{readinessError&&<div className="notice">{t(readinessError)}</div>}{!readyToShip&&<button type="button" className="secondary shipping-evidence-link" onClick={()=>document.getElementById('deal-evidence-vault')?.scrollIntoView({behavior:'smooth',block:'center'})}><ShieldCheck size={17}/>{t('Upload required evidence')}</button>}</div>}{shipment?<div className="shipment-card"><PackageCheck/><div><b>{shipment.carrier}</b><span>{t('Tracking number:')} {shipment.tracking_number}</span><small>{t(shipment.status==='delivered'?'Delivered':'Shipped')} · {formatDateTime(shipment.shipped_at)}</small></div></div>:deal.viewerRole==='seller'&&delivery?<form onSubmit={saveShipment}><label>{t('Carrier')}<input required minLength={2} value={carrier} onChange={e=>setCarrier(e.target.value)} placeholder={t('UPS, FedEx, USPS…')}/></label><label>{t('Tracking number')}<input required minLength={4} value={tracking} onChange={e=>setTracking(e.target.value)} placeholder={t('Enter tracking number')}/></label><button className="primary" disabled={!readyToShip||carrier.trim().length<2||tracking.trim().length<4}>{t('Mark as shipped')}</button></form>:deal.viewerRole==='buyer'&&delivery?<div className="shipping-wait">{t('Waiting for the seller to add tracking information.')}</div>:null}{shipment?.status==='shipped'&&deal.status==='accepted'&&<InspectionRecorder deal={deal} session={session} onRecorded={setInspectionRecorded}/>} {shipment?.status==='shipped'&&deal.viewerRole==='buyer'&&deal.status==='accepted'&&<button className="primary confirm-delivery" disabled={!inspectionRecorded} onClick={delivered}><PackageCheck size={18}/>{t('Confirm delivery')}</button>}{message&&<div className="notice">{t(message)}</div>}<p className="shipping-note"><ShieldCheck/> {t(inspectionRecorded?'Inspection recorded. Delivery can now be confirmed.':'Confirm delivery only after receiving and inspecting the item.')}</p></section>
}
...
function CreateDealReview({draft,photos,creating,onEdit,onSaveDraft,onPublish}:{draft:DealDraft;photos:File[];creating:boolean;onEdit:()=>void;onSaveDraft:()=>void;onPublish:()=>void}){
  ...
  return <section className="draft-review">
    ...
    <p className="lede small">{t('Check every detail before creating the public Deal Link.')}</p>
    <div className="draft-review-grid">
      ...
      <aside className="draft-review-summary">
        ...
        <SellerDeclarationChecklist id="seller-declarations" value={declarations} onChange={setDeclarations}/>
        <p><LockKeyhole/>{t('The Deal Link is not public until you confirm.')}</p>
        {!declarationsComplete&&<small className="declaration-required">{t('Confirm all declarations before publishing.')}</small>}
        <div className="draft-review-actions"><button className="secondary" disabled={creating} onClick={onEdit}>{t('Edit details')}</button><button className="secondary" disabled={creating} onClick={onSaveDraft}>{t('Save as draft')}</button><button className="primary" disabled={creating||!declarationsComplete} onClick={onPublish}>{t(creating?'Publishing…':'Confirm and publish')}<ArrowRight size={18}/></button></div>
      </aside>
    </div>
    <div className="create-review-dock" role="region" aria-live="polite" aria-label={t('Publish deal actions')}>
      <div><small>{t('Final step')}</small><strong>{t(declarationsComplete?'Ready to publish':'Seller confirmation required')}</strong><span>{t(declarationsComplete?'Your deal details are ready.':'Complete the 3 seller declarations to continue.')}</span></div>
      <button type="button" className="secondary" disabled={creating} onClick={onEdit}>{t('Edit')}</button>
      <button type="button" className="primary" disabled={creating} onClick={()=>{if(declarationsComplete){onPublish();return}const checklist=document.getElementById('seller-declarations');checklist?.scrollIntoView({behavior:'smooth',block:'center'});checklist?.setAttribute('tabindex','-1');checklist?.focus({preventScroll:true})}}>{t(creating?'Publishing…':declarationsComplete?'Confirm and publish':'Complete declarations')}<ArrowRight size={18}/></button>
    </div>
  </section>
}
  ...
  return <>
    <section className="global-hero">
      <div className="global-hero-copy">
        <p className="global-kicker"><ShieldCheck size={17}/>{t('Built for private deals in the United States')}</p>
        <h1>{t('Private deals,')}<br/><span>{t('handled with clarity.')}</span></h1>
        <p className="global-lede">{t('Agree on the terms, follow the payment status, and document the handoff in one private Deal Link.')}</p>
        ...
        <p className="beta-payment-note"><ShieldAlert size={16}/>{t('Sandbox demo — no real money is transferred. DealSafe is not legal escrow.')}</p>
      </div>
      <div className="network-stage" aria-label={t('DealSafe protected transaction flow')}>
        ...
        <article className="signal-card signal-payment"><span><BadgeDollarSign/></span><div><small>{t('PAYMENT')}</small><b>{t('Status visible')}</b></div><BadgeCheck/></article>
      </div>
    </section>
...
const publicInfoContent:Record<PublicInfoView,{eyebrow:string;title:string;intro:string;sections:Array<{title:string;body:string}>}>={
  'buyer-protection':{
    ...
    intro:'DealSafe keeps the accepted terms, payment state, delivery evidence, inspection record, and dispute history together.',
    sections:[
      ...
      {title:'Payment status stays visible',body:'The beta uses Stripe Sandbox. DealSafe does not store card or bank details and is not legal escrow.'},
  ...
  fees:{
    ...
    sections:[
      ...
      {title:'What the final quote should show',body:'Item price, DealSafe fee, payment processing, shipping, insurance, applicable taxes, and the final U.S. dollar amount.'},
  ...
  disputes:{
    ...
    intro:'DealSafe keeps the dispute reason, messages, delivery evidence, inspection record, and financial resolution together.',
    sections:[
  ...
  terms:{
    ...
    intro:'DealSafe is a private beta for recording transaction facts, consent, payment state, evidence, and handoff activity.',
    sections:[
  ...
  privacy:{
    ...
    intro:'DealSafe is designed to keep transaction information in one private record and avoid exposing personal contact details unnecessarily.',
    sections:[
      {title:'Sensitive payment data',body:'Card and bank details are handled by the payment provider and are not stored by DealSafe.'},
...
const getPageMetadata=(view:View,activeDeal?:Deal):PageMetadata=>{
  if(view==='home')return {
    label:'Home',
    title:'DealSafe — Safer Private Deals',
  ...
  if(view==='verify')return {
    ...
    title:'Verify an Agreement — DealSafe',
    description:'Compare a saved DealSafe agreement code without signing in.',
    path:verifyPath,
    indexable:true
  };
  if(Object.prototype.hasOwnProperty.call(publicInfoContent,view)){
    const content=publicInfoContent[view as PublicInfoView];
    return {label:content.title,title:`DealSafe — ${content.title}`,description:content.intro,path:publicInfoPaths[view as PublicInfoView],indexable:true};
  }
  const label=view==='deal'&&activeDeal?.title?activeDeal.title:(privateViewLabels[view]||'Private workspace');
  return {
    label,
    title:`${label} — DealSafe`,
    description:'Private DealSafe workspace for protected transaction records.',
    path:'/',
    indexable:false
  };
};
...
function App() {
  ...
  const [view,setView]=useState<View>(recoveryToken?'reset':viewFromPath()); const [deals,setDeals]=useState<Deal[]>([]); const [active,setActive]=useState<Deal>(); const [draft,setDraft]=useState(initial); const [buyer,setBuyer]=useState('');
  ...
  return <div className={`app view-${view}`}>
    ...
    <header className="site-header"><div className="header-inner">
      <div className="header-brand-group"><button className="brand" onClick={()=>goHomeSection()}><span><ShieldCheck size={21}/></span>DealSafe</button><span className="beta">{t('U.S. private beta')}</span></div>
...
const renderApp=()=>createRoot(document.getElementById('root')!).render(<React.StrictMode><App/></React.StrictMode>);
void initializeI18n().catch(()=>{}).then(renderApp);
