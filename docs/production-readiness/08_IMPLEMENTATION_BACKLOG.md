...
| FND-004 | P0 | **Complete locally for review:** exact dependencies, license and install-script policy, high-severity audit, repository secret scan, deterministic CycloneDX SBOM, bounded provenance manifest, CodeQL SAST, scoped ownership, finding SLAs, and CI retention exist; branch-protection enforcement and independent security-approver assignment remain external activation gates | Findings are visible and have ownership/SLA |
