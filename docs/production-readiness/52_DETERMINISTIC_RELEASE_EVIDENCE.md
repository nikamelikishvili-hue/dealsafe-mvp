...
- the fixed set of checks that precede evidence creation, including the
  ...
  the runtime-configuration contract, legacy-identifier policy, and
...
JavaScript and CSS asset. The exact legacy-identifier policy and verifier are
also required. A manifest that omits
