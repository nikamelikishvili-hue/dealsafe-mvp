...
test('release snapshot names the current reconciled no-go evidence', () => {
  ...
  assert.match(snapshot, /Status date: 2026-09-07/);
  assert.match(snapshot, /reviewed baseline `main` is `076be82bfa339ffee2ad9f0a7730cc081a85bd5b`/i);
  assert.match(snapshot, /415 foundation tests,\s+21 rendered-component tests/);
  assert.match(snapshot, /Initial application JavaScript is 133,643 bytes/);
  assert.match(snapshot, /DEALIVRA_STAGING_DATABASE_URL/);
  assert.match(snapshot, /No pull request remains open at this audit boundary/);
  assert.match(snapshot, /all 14 supported SPA\s+routes/);
  assert.match(snapshot, /dealsafe-9b9028nkn-nika13\.vercel\.app/);
  assert.match(snapshot, /served-asset-verification/);
  assert.match(snapshot, /34136690776/);
  assert.match(snapshot, /29 assets totaling\s+1,116,568 bytes/);
  assert.match(snapshot, /current exact-candidate\s+rerun remains required/);
...
test('shared UI foundations expose semantic tokens and accessible feedback states', () => {
  ...
  const agreementVerificationStyles = readText('src/agreement-verification.css');
  ...
  assert.match(shippingStyles, /\.shipping-panel,\.shipment-card,\.shipping-readiness\{[^}]*border:1px solid var\(--color-border-default\)[^}]*background:var\(--color-surface-card\)[^}]*box-shadow:var\(--shadow-card\)/);
  ...
  assert.match(agreementVerificationStyles, /\.agreement-verifier\{[^}]*background:var\(--color-ink-900\)[^}]*color:var\(--color-surface-card\)/);
  assert.match(agreementVerificationStyles, /\.agreement-verifier-result\.success\{background:var\(--color-success-100\);color:var\(--color-success-700\)\}/);
  assert.match(agreementVerificationStyles, /\.agreement-verifier-result\.warning\{background:var\(--color-warning-100\);color:var\(--color-warning-800\)\}/);
  assert.match(agreementVerificationStyles, /\.agreement-verifier-result\.error\{background:var\(--color-danger-100\);color:var\(--color-danger-800\)\}/);
  assert.doesNotMatch(agreementVerificationStyles, /#[0-9a-f]{3,8}\b|rgba?\(|hsla?\(/i);
  assert.match(dealExpiryStyles, /\.deal-expiry\{[^}]*border:1px solid var\(--color-warning-border\)[^}]*background:var\(--color-warning-100\)/);
  assert.match(dealExpiryStyles, /\.deal-expiry\.expired\{border-color:var\(--color-danger-border\)\}/);
  assert.match(dealExpiryStyles, /\.expired-agreement\{[^}]*border:1px solid var\(--color-danger-border\)[^}]*border-radius:var\(--radius-control\)/);
  ...
  assert.match(workflowStyles, /\.view-deal \[aria-invalid="true"\]\{border-color:var\(--color-danger-border\)!important\}/);
  assert.match(workflowStyles, /\.view-deal \.arrival-grid \.done\{[^}]*border-color:var\(--color-success-border\);[^}]*background:var\(--color-success-100\);[^}]*color:var\(--color-success-700\)/);
  assert.match(workflowStyles, /\.view-deal \.inspection-items label\.checked\{[^}]*border-color:var\(--color-success-border\);[^}]*background:var\(--color-success-100\);[^}]*color:var\(--color-success-700\)/);
  assert.match(workflowStyles, /\.view-deal \.pin-box\{background:linear-gradient\(145deg,var\(--color-ink-950\),var\(--color-info-800\)\)\}/);
  assert.doesNotMatch(workflowStyles, /#[0-9a-f]{3,8}\b|rgba?\(|hsla?\(/i);
