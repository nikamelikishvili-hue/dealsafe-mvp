...
begin
  ...
  values(new.id,1,jsonb_build_object('title',new.title,'description',new.description,'price_cents',new.price_cents,'currency',new.currency,'condition',new.condition,'delivery_method',new.delivery_method,'expires_at',new.expires_at),encode(extensions.digest(concat_ws('|',new.title,new.description,new.price_cents,new.currency,new.condition,new.delivery_method,new.expires_at),'sha256'),'hex'),new.seller_id)
