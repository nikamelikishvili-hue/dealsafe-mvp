...
Use `vercel dev` when testing real sign-in because production authentication is handled by the
same-origin `/api/auth` functions rather than exposing a long-lived refresh token to browser code.
